...
public class Controller {
	...
	public void ajouterPhoto() throws IOException {
		...
		if (photo != null) {
			...
			init.getCurrentPage().getListePhotos().get(init.getCurrentPage().getListePhotos().size() - 1)
					.drawPhoto(init);
		}
	}
	...
	public void refreshView() {
		...
		Text t = new Text(624, 635, String.valueOf(init.getCurrentPage().getNumero() + 1) + "/"
				+ String.valueOf(init.getAlbum().getListePages().size()));
		init.getRoot().getChildren().addAll(t);

		// On affiche ou non les flèches de navigation
		init.getRoot().lookup("#fleche_gauche").setManaged(true);
		if (init.getCurrentPage().getNumero() == 0) {
			init.getRoot().lookup("#fleche_gauche").setVisible(false);
		} else {
			init.getRoot().lookup("#fleche_gauche").setVisible(true);
		}

		if (init.getCurrentPage().getNumero() == init.getAlbum().getListePages().size() - 1) {
			init.getRoot().lookup("#fleche_droite").setVisible(false);
		} else {
			init.getRoot().lookup("#fleche_droite").setVisible(true);
		}
